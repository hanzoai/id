...
/**
 * The sign-in column, top to bottom.
 *
 ...
 * value in a pure module, it is the only part of the column a test can actually
 * hold still.
 *
 * `form` — the credential form, email or username first — LEADS. It is the one
 * way in every account has, and the page opens on it: the field, the password,
 * Continue, the code switch under it. A rule reading "or" follows the form, and
 * every other way in sits under that rule. There is no second arrangement for
 * any brand or page; login, signup and device approval all read this list.
 *
 * Google and GitHub head the entries under the rule because they are the
 * accounts most people arrive already signed into. A provider absent from this
 * list does not render at all, which is deliberate: a name here is the statement
 * that the portal knows how to draw and finish that provider's flow.
 *
 * `web3` and `phone` are not federated providers — the wallet is a capability of
 * the IAM binary and the phone selects which identifier the form asks for. They
 * live in this order anyway because the order is the COLUMN's, not the provider
 * list's: a person reading a column of ways to sign in does not care which of
 * them is an OAuth hop. Phone is last because it starts nothing; it re-labels
 * the field at the top.
 */
export const PROVIDER_ORDER = ['form', 'google', 'github', 'gitlab', 'web3', 'phone'] as const
